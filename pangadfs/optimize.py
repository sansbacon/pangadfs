...
class OptimizeMultilineupSets(OptimizeBase):
    """Optimizes lineup sets as first-class citizens for true multilineup optimization"""

    def optimize(self, ga: GeneticAlgorithm, **kwargs) -> Dict[str, Any]:
        """
        Optimizes for multiple diverse lineups using set-based approach
        ...
        logging.info(f'Creating initial population of {pop_size} lineup sets with {target_lineups} lineups each')
        ...
        # Validate each lineup in each set for salary constraints
        ...
        # Create new generations using set-based GA
        n_unimproved = 0
        ...
        for i in range(1, ga.ctx['ga_settings']['n_generations'] + 1):
            ...
            if ga.ctx['ga_settings'].get('verbose'):
                logging.info(f'Starting generation {i}')
                logging.info(f'Best set fitness {best_fitness}')
            ...
            if generation_best_fitness > best_fitness:
                logging.info(f'Set fitness improved to {generation_best_fitness}')
        ...
        logging.info(f'Set-based optimization complete. Generated {len(best_lineups)} diverse lineups.')
    ...
    def _validate_lineup_sets(self, 
        ...
        Remove sets that contain invalid lineups
        """
