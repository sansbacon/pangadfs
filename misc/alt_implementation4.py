...
def create_pool(csvpth):
    ...
    pool = pool.sort_values(['pos', 'proj'], ascending=[True, False]).reset_index(drop=True)
...
def multidimensional_shifting(elements, num_samples, sample_size, probs):
    ...
    Args:
        elements (iterable): iterable to sample from, typically a dataframe
    ...
    # row_array is the element number, not the index
    row_array = np.argpartition(shifted_probabilities, sample_size, axis=1)[:, :sample_size]

    # this converts element number to the index value
    return np.apply_along_axis(lambda x: elements.iloc[x].index, axis=1, arr=row_array)
...
def populate(pospool, posmap, num_samples, probcol='ppd'):
    ...
    pos_samples = {
      pos: multidimensional_shifting(pospool[pos], num_samples, n, pospool[pos][probcol])
